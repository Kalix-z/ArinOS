...
size_t fatFileSize(FHANDLE file) {
    if (file != nullptr)
...
FHANDLE fatCreate(const char* path, const char* name, const char* ext) {
    ...
    while (true) {
        read_sectors_ATA_PIO((size_t)fatSector, fdata.firstFatSec + k + START_SECTOR, 1);
        ...
        if (*_clusterNum == 0) {
            ...
            *_clusterNum = FAT16_CLUSTER_LAST_MIN;
            write_sectors_ATA_PIO(fdata.firstFatSec + k + START_SECTOR, 1, (size_t*)fatSector);
            break;
        }
...
void fatFileRead(FHANDLE file, char* _buf, size_t sz, size_t off) {
    char* buf = _buf;
    fat_file_handle* handle = (fat_file_handle*)file;

    size_t numOfClusters = sizeToCluster(sz);

    size_t offsetCluster = sizeToCluster(off) - 1;

    size_t bytesRemaining = sz;
    ...
    for (int i = 0; i < numOfClusters; i++) {
        ...
        if (i < offsetCluster) {
            ...
            read_sectors_ATA_PIO((size_t)FATTable, fatSector + START_SECTOR, 1);
        ...
        //read_sectors_ATA_PIO((size_t)buf, clusterToSector(activeCluster) + START_SECTOR, 1);
...
bool invalidFile(fat_file_t* fp) {
    // the deleted byte is larger than the signed character length
    if ((uint8_t)fp->lfn_order != 0x41)
        return true;
    return false;
}

void fatFileWrite(FHANDLE file, const char* buf, size_t sz, size_t off) {
    fat_file_handle* handle = (fat_file_handle*)file;

    size_t s = clusterToSector(handle->clusterNum) + START_SECTOR;
...
void reserveCluster(uint16_t* outCluster, size_t* outSector) {
    ...
    while(true) {
        ...
        read_sectors_ATA_PIO((size_t)fatBuf, fdata.firstFatSec + k + START_SECTOR, 1);
        ...
        if (*pcluster == 0) {
            *pcluster = FAT16_CLUSTER_LAST_MIN;
            write_sectors_ATA_PIO(fdata.firstFatSec + k + START_SECTOR, 1, (size_t*)fatBuf);
            *outCluster = i;
...
fat_file_t* findFileInDir(fat_file_t* fp, std::string name) {
    ...
    // dont bother with the Dot Entry files
    if (fp->name[0] == DOT_ENTRY)
        fp++;
    
    while (isValidFile(fp) && (char*)fp < orig + CLUSTER_BYTES) {
        
        if (fatFileDeleted(fp)) {
            fp++;
            continue;
        }

        if (invalidFile(fp)) {
            fp = (fat_file_t*)(((char*)fp)+0x20);
        }
        
        bool dir = isDir(fp);
        std::string fname = fatToStr(fp->name, dir);
...
std::string fatToStr(const char* fatName, bool dir) {
    std::string cname;
...
size_t clusterToSector(uint16_t clusterNum) {
    return ((clusterNum - 2) * fdata.secPerCluster) + fdata.firstDataSec;
}

uint16_t sectorToCluster(size_t sector) {
    return ((sector - fdata.firstDataSec) / fdata.secPerCluster) + 2;
}

void* readCluster(size_t sectorNum, char* buf) {
    char* orig = buf;
    for (int i = 0; i < SEC_PER_CLUSTER; i++) {
        read_sectors_ATA_PIO((size_t)orig, sectorNum+i, 1);
        orig += SECTOR_SIZE;
    }
    return buf;
}

void writeCluster(size_t sectorNum, char* buf) {
    char* orig = buf;
    for (int i = 0; i < SEC_PER_CLUSTER; i++) {
        write_sectors_ATA_PIO(sectorNum+i, 1, (size_t*)orig);
        orig += SECTOR_SIZE;
    }
}
