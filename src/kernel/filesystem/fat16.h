...
struct fat_data {
	...
	size_t    firstRootDirSec;
	size_t    firstFatSec;
	size_t    rootDirSecCount;
	size_t    fatSize;
};
...
inline static void readBootSec(char* ptr) {
    read_sectors_ATA_PIO((size_t)ptr, START_SECTOR, 1);
}

inline static size_t sizeToCluster(size_t sz) {
	const size_t clusterSz = SEC_PER_CLUSTER * SECTOR_SIZE;
...
void fatFileRead(FHANDLE file, char* buf, size_t sz, size_t off);
